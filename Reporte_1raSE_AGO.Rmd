...
* Dra. Dalia Suárez Salazar, Jefa de Gabinete de Asesores MINSA. 


* Elaborado por: Dr. José Manuel Magallanes, Asesor en  Inteligencia Estratégica del Despacho Ministerial. 


## Período de Reporte:

Primera semana de Agosto 2021
...
SEMANAtxt="Hasta Primera Semana de agosto"
histIndex=grep("feb|mar|abr|may|jun|jul|ago",names(datamodel))
...
#simplify
library(kableExtra)
link2='https://docs.google.com/spreadsheets/d/e/2PACX-1vT-v8E1ANTx81cNq6vtW73xYc_zJoVXMvYlMbmu4glxHYLrd8IpF_vyTXhoMGuELPwhjEJLO0Hr_RSu/pub?gid=15394421&single=true&output=csv'
...
basemap1= ggplot(data = regmap) +theme_void() +geom_sf(fill="grey97",lwd=0.2)
...
Las provincias vuelve a categorizarse. 
...
```{r, echo=FALSE}
last=length(histIndex)
actual=histIndex[last]
antes=histIndex[last-1]


table(datamodel[,actual],datamodel[,antes],dnn = c("ACTUAL","ANTERIOR")) %>%
  ftable()%>%pander::pander()
```


```{r,echo=FALSE}
mejor=sum(datamodel[,actual]<datamodel[,antes])
igual=sum(datamodel[,actual]==datamodel[,antes])
peor=sum(datamodel[,actual]>datamodel[,antes])
```
...
map1=basemap1 + geom_sf(aes(fill=Escenario_Final),color="grey70",lwd=0.5) 
