...
sindat_anual_prov=rbind(sindat_anual_prov1,sindat_anual_prov2)
...
```{r}
ggplot() +geom_line(data=sindat_anual_prov1[sindat_anual_prov1$PROVINCIA%in%sel2020,],aes(semana,morbilidad,color=PROVINCIA)) + theme( legend.title = element_blank() ) + guides(color=FALSE)  #+ scale_y_log10()
```

```{r}
ggplot() +geom_line(data=sindat_anual_prov2[sindat_anual_prov2$PROVINCIA%in%sel2021,],aes(semana,morbilidad,color=PROVINCIA)) + theme( legend.title = element_blank() ) + guides(color=FALSE) # + scale_y_log10()
```
...
```{r}
select=c("rk2017","rk2018","rk2019","rk2020","rk2021","pareto2020")
dfParetoPlot=dfPareto[,c("PROVINCIA",select)]
dfParetoPlot$pareto2020=as.factor(dfParetoPlot$pareto2020)
library(GGally)
...
# Plot
p=ggparcoord(dfParetoPlot,scale = "globalminmax",
    columns = 2:6,groupColumn = 7
    ) + 
       geom_text(data = dfParetoPlot %>%
              select(PROVINCIA) %>%
              mutate(x = 1,
                     y = dfParetoPlot$rk2017),
...
#mostProv=Reduce(intersect,list(provsbad2017,provsbad2018,
                      #provsbad2019,provsbad2020,provsbad2021))$PROVINCIA

# mapapro=sf::read_sf("PROVINCIAS.shp")
# 
# mapapro$worst=ifelse(mapapro$PROV2%in%mostProv,1,0)
# 
# setdiff(provsbad2019$PROVINCIA,mostProv)
# 
# mapapro[mapapro$worst==1,]
```
...
allCovid=sinall[sinall$CORONAVIRU==1,]
