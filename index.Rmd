...
#library(ggpmisc)
#
pareto20_1=ggplot(data=sindat_anual_prov1pareto[sindat_anual_prov1pareto$gruposem=="Antes semana 20",],
             aes(semana,fallecidos,color=PROVINCIA)) +geom_line() +
 #stat_peaks(colour = "red",span = 10,strict = T)+
theme( legend.title = element_blank() ) + guides(color=FALSE) + scale_y_log10() + labs(title = "Pico antes de semana 20")
pareto20_1=pareto20_1%>%plotly::ggplotly()
```


```{r}
pareto20_2=ggplot(data=sindat_anual_prov1pareto[sindat_anual_prov1pareto$gruposem=="Sem 20-24",],
             aes(semana,fallecidos,color=PROVINCIA)) +geom_line() +
 #stat_peaks(colour = "red",span = 10,strict = T)+
theme( legend.title = element_blank() ) + guides(color=FALSE) + scale_y_log10() + labs(title = "Pico entre semanas 20-24")
pareto20_2=pareto20_2%>%plotly::ggplotly()
```
...
pareto20_3=pareto20_3%>%plotly::ggplotly()
```
...
pareto20_4=ggplot(data=sindat_anual_prov1pareto[sindat_anual_prov1pareto$gruposem=="Luego semana 30",],
             aes(semana,fallecidos,color=PROVINCIA)) +geom_line() +
 #stat_peaks(colour = "red",span = 10,strict = T)+
theme( legend.title = element_blank() ) + guides(color=FALSE) + scale_y_log10() + labs(title = "Pico luego semana 30")
pareto20_4=pareto20_3%>%plotly::ggplotly()
```


```{r}

pareto21_1=ggplot(data=sindat_anual_prov2pareto[sindat_anual_prov2pareto$gruposem=="Antes semana 5",],
             aes(semana,fallecidos,color=PROVINCIA)) +geom_line() +
 #stat_peaks(colour = "red",span = 20,strict = T)+
theme( legend.title = element_blank() ) + guides(color=FALSE) +  scale_y_log10() + labs(title = "Pico antes semana 5")
pareto21_1=pareto21_1%>%plotly::ggplotly()
```
...
pareto21_2=pareto21_2%>%plotly::ggplotly()
```
...
pareto21_3=pareto21_3%>%plotly::ggplotly()
```

```{r}

pareto21_4=ggplot(data=sindat_anual_prov2pareto[sindat_anual_prov2pareto$gruposem=="Luego semana 14",],
             aes(semana,fallecidos,color=PROVINCIA)) +geom_line() +
 #stat_peaks(colour = "red",span = 20,strict = T)+
theme( legend.title = element_blank() ) + guides(color=FALSE) +  scale_y_log10()+ labs(title = "Pico luego semana 14")
pareto21_4=pareto21_4%>%plotly::ggplotly()
```
...
line1=line  +  scale_x_continuous(breaks = 1:18,limits = c(1,18)) + scale_y_log10() +geom_vline(xintercept = c(7,9),color=c("grey","black"),linetype=2) + geom_text(x=7,y=1,label="Vacuna\nSalud",angle=90)  + geom_text(x=9,y=1,label="Vacuna\nSeguridad",angle=90)
...
line3=line  +  scale_x_continuous(breaks = 1:18,limits = c(1,18))+ scale_y_log10() +geom_vline(xintercept = c(7,9),color=c("grey","black"),linetype=2) + geom_text(x=7,y=1,label="Vacuna\nSalud",angle=90)  + geom_text(x=9,y=1,label="Vacuna\nSeguridad",angle=90)
...
line4=line4 + theme(legend.position = "none") +  scale_x_continuous(breaks = 1:18,limits = c(1,18)) +labs(title=" ",y="Conteo",color=" ")  +geom_vline(xintercept = c(7,9),color=rep(c("grey","black"),3),linetype=2) 
...
line6=line6 + theme(legend.position = "none") +  scale_x_continuous(breaks = 1:18,limits = c(1,18)) +labs(title=" ",y="",color=" ") + scale_y_log10()
...
suben=c("PIURA","LA LIBERTAD","ANCASH","PUNO","AYACUCHO","TACNA", "CUSCO","CAJAMARCA","AMAZONAS","AREQUIPA","JUNIN", "APURIMAC")
estables=c("SAN MARTIN", "LAMBAYEQUE","ICA", "HUANUCO", "HUANCAVELICA","MOQUEGUA","PASCO")

bajan=setdiff(allCovid_geo$DEPARTAMEN,union(suben,estables))
geodata=allCovid_geo[allCovid_geo$anosem<=202118,]
